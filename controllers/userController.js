                ...
                if (results.length == 1){//Akun dengan parameter email ditemukan
                    ...
                    User.update(
                        ...
                        if(err){
                            ...
                            return res.json({success: true, data: {message:'Sandi yang sudah diatur kembali sudah dikirim ke email yang Anda tuliskan.'}})
                        }
                    })

                }else{
                    return res.json({success: true, data: {message:"Email yang Anda tuliskan tidak ditemukan dalam database."}})
                }
            }
        })
}

exports.atur_ulang_sandi_manual = function (req, res) {
    ...
    req.checkBody('sandi', 'Sandi tidak boleh kosong').notEmpty();
    ...
    if (errors) {//Terjadinya kesalahan
        ...
        rClient.post(base_api_general_url + '/cek_session', args, function (data, response) {
            if (data.success == true) {//session berlaku
                ...
                //Fungsi mengubah sandi
                var sandiMurni = req.body.sandi
                var sandiSalt = md5(sandiMurni + salt_password)
                ...
                User.update(
                    ...
                    if (err) {
                        ...
                        return res.json({
                            success: true,
                            data: {message: 'Sandi berhasil diubah.'}
                        })
                    }
                })
      ...
      if(data.success == true){//session berlaku
        ...
        UserSiswa.update(
          ...
          if(err){
           ...
           return res.json({success: true, data: {message:'Bahasa yang anda kuasai berhasil ditambahkan.'}})
         }
        })
  ...
  if(errors){//Terjadinya kesalahan
    ...
    rClient.post(base_api_general_url+'/cek_session', args, function (data, response) {
      if(data.success == true){//session berlaku
        ...
        //Mengubah data nama lengkap dan bio
        UserSiswa.update({ _id: req.body.pengguna }, { $set: { 'media_sosial.facebook': req.body.facebook, 'media_sosial.instagram': req.body.instagram, 'media_sosial.twitter': req.body.twitter }})
         .exec(function (err, results) {
             if (err) {
               return res.json({success:false, data: {message:err}})
             }else{
               return res.json({success:true, data: {message:'Data media sosial anda berhasil diperbaharui.'}})
             }
         })
  ...
  if(errors){//Terjadinya kesalahan
    ...
    rClient.post(base_api_general_url+'/cek_session', args, function (data, response) {
      if(data.success == true){//session berlaku
        ...
        //Mengubah data nama lengkap dan bio
        User.update({ _id: req.body.pengguna }, { $set: { 'profil.nama_lengkap': req.body.nama_lengkap, 'profil.bio': req.body.bio }})
         .exec(function (err, results) {
             if (err) {
               return res.json({success:false, data: {message:err}})
             }else{
...
exports.sekolah_profil_ubah = function(req,res) {
...
exports.sekolah_foto_profil_ubah = function(req,res) {
