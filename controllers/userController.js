...
exports.get_profile = function(req,res,next) {
  ...
  if(req.body.username == '' || req.body.access_token == '' || req.body.username == null || req.body.access_token == null){
    return res.json({success: false, data: {pesan:'Username atau akses token tidak boleh kosong'}})
  }else{
          ...
          two: function(callback){
            ...
            setTimeout(function() {
              if (err) {
                ...
                if(expire == true){
                  return res.json({success: false, data: {pesan:'Akses token sudah expired atau tidak ditemukan'}})
                }else if(expire == false){
                  if(results.length == 0){
                    return res.json({success: false,data:{pesan:'Data profil tidak ditemukan'}})
                  }else{
